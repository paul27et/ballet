...
const Carousel = (p) => {
    ...
    return (<Dynamic component={local.as} ref={mergedRef} {...props} onKeyDown={handleKeyDown} onMouseOver={handleMouseOver} onMouseOut={handleMouseOut} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} className={classNames(local.className, prefix, local.slide && "slide", local.fade && `${prefix}-fade`, local.variant && `${prefix}-${local.variant}`)}>
      {local.indicators && (<div className={`${prefix}-indicators`}>
          <For each={items()}>
            {(_, index) => (<button type="button" data-bs-target="" // Bootstrap requires this in their css.
             aria-label={local.indicatorLabels?.length
                    ...
                    : `Slide ${index() + 1}`} className={isActive(index()) ? "active" : undefined} onClick={(e) => onSelect?.(index(), e)} aria-current={isActive(index())}/>)}
          </For>
        </div>)}
      ...
      {local.controls && (<>
          {(local.wrap || activeIndex() !== 0) && (<Anchor className={`${prefix}-control-prev`} onClick={prev}>
          ...
          {(local.wrap || activeIndex() !== items().length - 1) && (<Anchor className={`${prefix}-control-next`} onClick={next}>
